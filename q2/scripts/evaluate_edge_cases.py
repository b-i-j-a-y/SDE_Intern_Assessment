...
def main():
    ...
    edge_case_dir = project_root / "data" / "edge_cases"

    output_root = project_root / "q2" / "outputs" / "edge_cases"

    mask_dir = output_root / "person_masks"
    parsing_dir = output_root / "parsing_maps"
    agnostic_dir = output_root / "agnostic"
    ...
    # Only load image files (ignore CSV, TXT, etc.)
    images = sorted(
        [
            file
            for file in edge_case_dir.iterdir()
    ...
    for image_path in images:
        ...
        mask, score, _ = segmenter.segment(
            str(image_path),
            box
        )
        ...
        agnostic = agnostic_generator.generate(image_path)
